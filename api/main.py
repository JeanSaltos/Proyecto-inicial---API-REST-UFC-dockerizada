...
# El driver ODBC 18 es el estándar moderno para Linux
connection_string = (
    ...
    f"UID={DB_USER};"
    f"PWD={DB_PASSWORD};"
    "Encrypt=no;" # Deshabilitado para simplificar (no requiere SSL)
    "TrustServerCertificate=yes;" # Necesario si Encrypt=no
)
...
def startup_event():
    ...
    try:
        ...
        # En un escenario real, podrías querer que la app falle si no puede conectar.
...
class Fighter(FighterBase):
    ...
    class Config:
        from_attributes = True # Permite mapear desde objetos de la DB
...
def get_fighters():
    ...
    try:
        ...
        # Convertir las filas (pyodbc.Row) a diccionarios para Pydantic
        for row in rows:
